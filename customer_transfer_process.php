...
<?php
   ...
   //select the last transaction id in the sender's passbook
    ...
    if($s_amount<=500)  {
        echo '<script>alert("Πρέπει να έχετε τουλάχιστον 500€ στον λογαριασμό σας για να συνεχίσετε με τη συναλλαγή.");';
        echo 'window.location= "customer_transfer.php";</script>';
    }
    elseif($t_amount<100){
         echo '<script>alert("Δε μπορείτε να μεταφέρετε λιγότερο από 100€");';
        echo 'window.location= "customer_transfer.php";</script>';
    }
    elseif($s_total<500){
        echo '<script>alert("Δεν έχετε το απαιτούμενο υπόλοιπο για να συνεχίσετε.\n\nΠρέπει να έχετε τουλάχιστον 500€ στον λογαριασμό σας.");';
        echo 'window.location= "customer_transfer.php";</script>';
    }  
    else{ 
        //insert statement into reciever passbook.
        $r_total=$r_amount+$t_amount;
        $sql1="insert into passbook".$reciever_id." values('','$date','$r_name','$r_branch','$r_ifsc','$t_amount','0','$r_total','$s_name')";
        mysqli_query($con,$sql1) or die(mysqli_error($con));
        
        //insert statement into sender passbook.
        $s_total=$s_amount-$t_amount;
        $sql2="insert into passbook".$sender_id." values('','$date','$s_name','$s_branch','$s_ifsc','0','$t_amount','$s_total','$r_name')";
         mysqli_query($con,$sql2) or die(mysqli_error($con));       
        echo '<script>alert("Επιτυχής συναλλαγή.");';
        echo 'window.location= "customer_transfer.php";</script>';
    }
?>